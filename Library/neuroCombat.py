...
def neuroCombat(dat,
    ...
    estimates = {'batches': info_dict['batch_levels'], 'var.pooled': v_pool, 'stand.mean': s_mean, 'mod.mean': mod_mean,
                 'gamma.star': gamma_star, 'delta.star': delta_star}
...
def standardize_across_features(X, design, info_dict):
    ...
    if ref_level is not None:
        ...
        var_pooled = np.dot(((X_ref - np.dot(design_ref, B_hat).T) ** 2),
